...
{   
	...
	public void getNextMove(GameStateModule state)
	{
        ...
        for (int i = 0; i < width; i++) {
            if (game.canMakeMove(order[i]))
                game.makeMove(order[i]); //max makes a move
            else
                continue;
        ...
        game = state.copy();
        while (!terminate) {
            MaxValue(game, 0, depthLimit, -Integer.MAX_VALUE, Integer.MAX_VALUE);
            depthLimit++;
            if (!terminate)
    ...
    int MaxValue(GameStateModule state, int depth, int depthLimit, int alpha, int beta) {
        ...
        for (int i = 0; i < width; i++) { 
            if (game.canMakeMove(order[i])) {
                game.makeMove(order[i]); 
            }
            else
                continue;
            int result = MinValue(game, depth+1, depthLimit, alpha, beta);
            if (value < result) {
                value = result;
                if (depth == 0) {
                    localChosenMove = order[i];
                }
            }
            if (value >= beta)
                return value;
    ...
    int MinValue(GameStateModule state, int depth, int depthLimit, int alpha, int beta) {
        if (state.isGameOver() || depth == depthLimit || terminate)
            return evalFunc(state);
        ...
        for (int i = 0; i < width; i++) {
            if (game.canMakeMove(order[i])) {
                ...
                continue;
            value = Math.min(value, MaxValue(game, depth+1, depthLimit, alpha, beta));
            if (value <= alpha)
                return value;
    ...
    private int evalFunc(GameStateModule state) {
